...
file_object = open("nbmodeltask3.txt", "r").readlines()
...
file_object = open("nboutputtask3.txt", "w")
...
def accuracy(spam_class = [], ham_class = [], classification = {}):
    ...
    for email in classification:
        ...
        if classification[email] == "ham" and email in ham_class:
            correctly_classified_count += 1
    accuracy = round((correctly_classified_count/len(emails)), 4)
    ...
    for textfile in classification:
        if classification[textfile] == class_name:
            ...
            if textfile in kclass:
                correctly_classified_count += 1
    class_precision = round((correctly_classified_count/classified_count), 4)
    print("Precision("+class_name+")" +": "+ str(class_precision))
    class_recall = round((correctly_classified_count/len(kclass)), 4)
    print("Recall("+class_name+")" +": "+ str(class_recall))
    fscore = round(((2*class_precision*class_recall)/(class_precision+class_recall)), 4)
